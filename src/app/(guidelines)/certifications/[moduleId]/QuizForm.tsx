...
export default function QuizForm({
    ...
    return (
      <div
        style={{
          padding: "1.25rem",
          borderRadius: "8px",
          border: `1px solid ${result.passed ? "var(--brand-blue)" : "var(--brand-pink)"}`,
          background: result.passed ? "var(--brand-blue-lt)" : "var(--brand-pink-lt)",
          color: result.passed ? "var(--brand-blue-dk)" : "var(--brand-pink)",
          fontSize: "0.95rem",
        }}
      >
        {result.text}
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      {result && !result.ok && (
        <p style={{ fontSize: "0.85rem", color: "var(--brand-pink)" }}>{result.text}</p>
      )}
    </form>
  );
}
