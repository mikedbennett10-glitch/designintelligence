...
const respColors: Record<string, { bg: string; fg: string }> = {
  OFOI: { bg: "var(--brand-blue-lt)", fg: "var(--brand-blue-dk)" },
  OFCI: { bg: "var(--brand-pink-lt)", fg: "var(--brand-pink)" },
  "IT/OFOI": { bg: "var(--brand-blue-lt)", fg: "var(--brand-blue-dk)" },
  "IT/OFCI": { bg: "var(--brand-pink-lt)", fg: "var(--brand-pink)" },
};

function RespBadge({ responsibility }: { responsibility: string }) {
  const c = respColors[responsibility] ?? { bg: "var(--brand-charcoal-lt)", fg: "var(--muted)" };
  return (
    <span
      style={{
        fontSize: "0.72rem",
