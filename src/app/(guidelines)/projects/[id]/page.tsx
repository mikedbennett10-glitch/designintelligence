...
const DEVIATION_STATUS_COLORS: Record<string, { bg: string; fg: string }> = {
  ...
  approved: { bg: "var(--brand-blue-lt)", fg: "var(--brand-blue-dk)" },
  approved_with_conditions: { bg: "var(--brand-blue-lt)", fg: "var(--brand-blue-dk)" },
  denied: { bg: "var(--brand-pink-lt)", fg: "var(--brand-pink)" },
};
  ...
  return (
    <div>
        ...
        {project.edition_lock_id ? (
          <div style={{ marginBottom: "1rem" }}>
            ...
            {!isCurrent && currentEditionName && (
              <p style={{ margin: "0.5rem 0 0", color: "var(--brand-pink)", fontSize: "0.85rem" }}>
      ...
      {project.edition_lock_id && delta.length > 0 && (
        ...
        >
          <h2 style={{ fontSize: "1rem", marginTop: 0, marginBottom: "0.4rem", color: "var(--brand-pink)" }}>
            ...
            {delta.map((d) => (
              ...
              >
                <Link
                  ...
                  style={{ fontWeight: 600, color: "var(--brand-pink)" }}
                >
                  {d.room_name}
                </Link>
        ...
        {deviations.length === 0 ? (
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none" }}>
            {deviations.map((d) => (
              ...
              >
                <span>
                  <Link href={`/ambulatory/rooms/${d.room_taxonomy_id}`} style={{ color: "var(--brand-blue-dk)", fontWeight: 600 }}>
                    {d.room_name}
                  </Link>
                ...
                <span
                  style={{
                    ...
                    borderRadius: "999px",
                    background: DEVIATION_STATUS_COLORS[d.status]?.bg ?? "var(--brand-charcoal-lt)",
          ...
          <ul style={{ margin: 0, paddingLeft: "1.25rem" }}>
            {roomsInScope.map((r) => (
              <li key={r.taxonomy_id} style={{ marginBottom: "0.25rem" }}>
                <Link href={`/ambulatory/rooms/${r.taxonomy_id}`} style={{ color: "var(--brand-blue-dk)" }}>
                  {r.name}
                </Link>
              </li>
            ))}
